...
def _get_batch_logps(logits: torch.FloatTensor, labels: torch.LongTensor, average_log_prob: bool = False) -> torch.FloatTensor:
    ...
    #print('per_token_logps----------------',per_token_logps.shape)
...
class BasicTrainer(object):
    ...
    def concatenated_forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        #print('concatenated_batch',concatenated_batch['concatenated_input_ids'].shape)
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name in {'dpo', 'ipo'}:
            ...
            #print('--------loss',losses, chosen_rewards, rejected_rewards)
        ...
        for batch in self.train_iterator:
            ...
            if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
                ...
                for eval_batch in (tqdm.tqdm(self.eval_batches, desc='Computing eval metrics') if self.rank == 0 else self.eval_batches):
                    ...
                    #print('local_eval_batch',local_eval_batch)
                    with torch.no_grad():
...
class FSDPTrainer(BasicTrainer):
    ...
    def save(self, output_dir=None, metrics=None):
        ...
        if self.rank == 0:
            self.write_state_dict(self.example_counter, policy_state_dict, metrics, 'policy.pt', output_dir)
            print('Write down the policy already')
        del policy_state_dict
        dist.barrier()
